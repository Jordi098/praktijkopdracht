    ...
    <form action="{{ route('story.store') }}" method="POST" enctype="multipart/form-data"
        ...
        <div class="flex flex-col">
            ...
            <textarea id="text" name="text" rows="5"
                      class="px-4 py-2 rounded-md border border-gray-600 bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">{{ old('text') }}</textarea>
            @error('text')
            <div class="text-red-500 text-sm mt-1">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="flex flex-col">
            ...
            <select name="category_id" id="category_id"
                    class="px-4 py-2 rounded-md border border-gray-600 bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
                <option value="" disabled {{ old('category_id') ? '' : 'selected' }}>-- Kies categorie --</option>
        ...
        <x-primary-button type="submit" class="text-white">Create</x-primary-button>
    </form>
</x-app-layout>
